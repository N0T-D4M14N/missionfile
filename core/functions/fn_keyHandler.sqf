    ...
    case 20: {
        if (!_alt && {!_ctrlKey} && {!dialog} && {!life_action_inUse} && {!(player getVariable ["playerSurrender",false])} && {!(player getVariable ["restrained",false])} && {!life_isknocked} && {!life_istazed}) then {
            if (!(isNull objectParent player) && alive vehicle player) then {
                ...
                private "_list";
                _list = ((ASLtoATL (getPosASL player)) nearEntities [["Box_IND_Grenades_F","B_supplyCrate_F","RL_HouseBox_5"], 2.5]) select 0;
                if (!(isNil "_list")) then {
                    _house = nearestObject [(ASLtoATL (getPosASL _list)), "House"];
                    if (_house getVariable ["locked", false]) then {
                        if (_house isKindOf "Land_Ryb_domek") then {
                            hint "Breche zuerst die Tür vom Drogenlabor auf!";
                        } else {
    ...
    case 22: {
        if (!_alt && !_ctrlKey) then {
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                ...
                private _restartSaved = _veh getVariable ["restart_saved",false];
                if (_veh in life_vehicles && {player distance _veh < 20}) then {
                    if (_locked isEqualTo 2 && (_restartSaved isEqualTo false)) then {
                        if (local _veh) then {
                            _veh lock 0;
                        ...
                        if (_restartSaved isEqualTo false) then {
                            ...
                            systemChat "Vehicle is saved for the Restart!"
                        };
                    };
                };
            };
