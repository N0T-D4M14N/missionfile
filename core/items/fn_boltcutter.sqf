...
if (!(_building isKindOf "House_F") && !(_building isKindOf "Land_Ryb_domek")) exitWith {hint localize "STR_ISTR_Bolt_NotNear";};
...
//systemChat (format [%1,(str _door)]);

if (_building isKindOf "Land_Ryb_domek") then
   { _door = 1; }
else {
    for "_i" from 1 to _doors do {
    _selPos = _building selectionPosition format ["Door_%1_trigger",_i];
...
if ((nearestObject [_pos,"Land_Dome_Big_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) then {
    ...
    [0,"STR_ISTR_Bolt_AlertHouse",true,[profileName]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
        ...
        if (_building isKindOf "Land_Ryb_domek") then {
            [[1,2],"STR_ISTR_Bolt_AlertDruglab",true,[]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
        };
    };
};
